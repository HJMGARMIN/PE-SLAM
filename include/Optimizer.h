...
public:
    ...
    int static PoseOptimization(KeyFrame* pCurKF);
    ...
    // Inertial pose-graph
    void static InertialOptimization(Map *pMap, Eigen::Matrix3d &Rwg, double &scale, Eigen::Vector3d &bg, Eigen::Vector3d &ba, bool bMono, Eigen::MatrixXd  &covInertial, bool bFixedVel=false, bool bGauss=false, float priorG = 1e2, float priorA = 1e6, bool isUpdate=true);
    ...
    // Gyroscope bias
    void static GyroscopeBiasOptimizer(Map *pMap, Eigen::Vector3d &bg, float &err);
    std::vector<Eigen::Matrix3d> static summation(const std::vector<Eigen::Vector3f> &bearingVectori, const std::vector<Eigen::Vector3f> &bearingVectorj);
    void static reduceVector(std::vector<Eigen::Vector3f> &v, std::vector<uchar> status);
    void static reduceVector(std::vector<cv::Point2f> &v, std::vector<uchar> status);
    double static distance(cv::Point2f &pt1, cv::Point2f &pt2);
    // Translation
    void static EstimateTranslation(const vector<KeyFrame*> &vpKFs);
    // Velocity and gravity
    bool static LinearLooselyCoupledAlignment(const vector<KeyFrame*> &vpKFs);
    //VINS
    void static SolveGyroscopeBiasVins(const vector<KeyFrame*> &vpKF, Map *pMap, Eigen::Vector3d &bg);
