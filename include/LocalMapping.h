...
public:
    ...
    void SaveTrajectory(vector<KeyFrame*> vpKF, const string out_file_name);
    void SaveGyroscopeBias(const Eigen::Vector3d &bg, const string out_file_name);
    void SaveComputationTime(const double &cost_t, const string out_file_name);
    string Double2String(double val, int fixed);
    // void LoadYAMLFile(const string & config_file);
...
protected:
    ...
    void InitializeIMU(float priorG = 1e2, float priorA = 1e6, bool bFirst = false, bool isUsed = false);
