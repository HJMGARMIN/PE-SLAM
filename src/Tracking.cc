...
Tracking::Tracking(System *pSys, ORBVocabulary* pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Atlas *pAtlas, KeyFrameDatabase* pKFDB, const string &strSettingPath, const int sensor, Settings* settings, const string &_nameSeq):
    ...
    mnInitialFrameId(0), mbCreatedMap(false), mnFirstFrameId(0), mpCamera2(nullptr), mpLastKeyFrame(static_cast<KeyFrame*>(NULL)), framePerSecond(0)
{
...
void Tracking::StereoInitialization()
{
    if(mCurrentFrame.N>500)
    {
        ...
        {
            double curKFTimeStamp = 1e9 * mCurrentFrame.mTimeStamp;
            string imgName = to_string(curKFTimeStamp);
            string imgName_prefix = imgName.substr(0, imgName.find("."));
            if (!mCurrentFrame.mpImuPreintegrated || !mLastFrame.mpImuPreintegrated)
            {
                cout << imgName_prefix <<"has not IMU meas" << endl;
                return;
            }

            if (!mFastInit && (mCurrentFrame.mpImuPreintegratedFrame->avgA-mLastFrame.mpImuPreintegratedFrame->avgA).norm()<0.5)
            {
                cout << imgName_prefix <<"not enough acceleration" << endl;
                return;
            }
        ...
        pKFini->AddImage(mCurrentFrame.imgLeft, mCurrentFrame.imgRight);
...
void Tracking::CreateNewKeyFrame()
{
    ...
    pKF->AddImage(mCurrentFrame.imgLeft, mCurrentFrame.imgRight);
