...
void Frame::ComputeStereoFromRGBD(const cv::Mat &imDepth)
{
    ...
    tbb::parallel_for(tbb::blocked_range<std::size_t> (0,N),[&](tbb::blocked_range<std::size_t> rN)
    {
        ...
        for(size_t i=rN.begin(); i<rN.end(); i++)
        {
...
void Frame::ComputeStereoFishEyeMatches() {
    ...
    tbb::parallel_for(tbb::blocked_range<int>(0,Nmatches),[&](tbb::blocked_range<int> rNmatcher)
    {
        for(int i = rNmatcher.begin(); i < rNmatcher.end(); i++)
        {
            if(matches[i].size() >= 2 && matches[i][0].distance < matches[i][1].distance * 0.7){
            //For every good match, check parallax and reprojection error to discard spurious matches
            Eigen::Vector3f p3D;
            descMatches++;
            float sigma1 = mvLevelSigma2[mvKeys[matches[i][0].queryIdx + monoLeft].octave], sigma2 = mvLevelSigma2[mvKeysRight[matches[i][0].trainIdx + monoRight].octave];
            float depth = static_cast<KannalaBrandt8*>(mpCamera)->TriangulateMatches(mpCamera2,mvKeys[matches[i][0].queryIdx + monoLeft],mvKeysRight[matches[i][0].trainIdx + monoRight],mRlr,mtlr,sigma1,sigma2,p3D);
            if(depth > 0.0001f){
                mvLeftToRightMatch[matches[i][0].queryIdx + monoLeft] = matches[i][0].trainIdx + monoRight;
                mvRightToLeftMatch[matches[i][0].trainIdx + monoRight] = matches[i][0].queryIdx + monoLeft;
                mvStereo3Dpoints[matches[i][0].queryIdx + monoLeft] = p3D;
                mvDepth[matches[i][0].queryIdx + monoLeft] = depth;
                nMatches++;
            }
        }
        }
