...
Sophus::SE3f System::TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    // int nKF1 = mpLocalMapper->KeyframesInQueue();
    std::chrono::steady_clock::time_point sfm_st =std::chrono::steady_clock::now();  
    Sophus::SE3f Tcw = mpTracker->GrabImageStereo(imLeftToFeed,imRightToFeed,timestamp,filename);
    // int nKF2 = mpLocalMapper->KeyframesInQueue();

    // if(nKF2 - nKF1 == 1)
    // {
        // std::cout<<"timestamp: "<<timestamp<<std::endl;
        std::chrono::steady_clock::time_point sfm_end = std::chrono::steady_clock::now();
        std::chrono::duration<double, std::milli>  sfm_dur = std::chrono::duration<double,std::milli>(sfm_end-sfm_st);
        double cost_t = sfm_dur.count();
        // std::cout<<"[Ours] SfM takes: "<<cost_t<<" ms"<<std::endl;
        mpTracker->framePerSecond = 1000.0 / cost_t;
    // }
...
Sophus::SE3f System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    if(nKF2 - nKF1 == 1)
    {
        ...
        double cost_t = sfm_dur.count();
        // std::cout<<"[Ours] SfM takes: "<<cost_t<<" ms"<<std::endl;
        mpTracker->framePerSecond = 1.0 / cost_t;
    }
...
void System::SaveTrajectoryEuRoC(const string &filename)
{
        ...
        lend=mpTracker->mlRelativeFramePoses.end();lit!=lend;lit++, lRit++, lT++, lbL++)
    {
        ...
        if (mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO || mSensor==IMU_RGBD)
        {
            ...
            f << setprecision(6) << 1e9*(*lT) << " " <<  setprecision(9) << twb(0) << " " << twb(1) << " " << twb(2) << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w()  << " " 
            << ba(0) << " "
            << ba(1) << " "
            << ba(2) << " "
            << bg(0) << " "
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
       ...
       // pKF->SetPose(pKF->GetPose()*Two);
        ...
        if (mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO || mSensor==IMU_RGBD)
        {
            ...
            Eigen::Vector3f bg = pKF->GetGyroBias();
            Eigen::Vector3f ba = pKF->GetAccBias();
            f << setprecision(6) << 1e9*pKF->mTimeStamp  << " " <<  setprecision(9) << twb(0) << " " << twb(1) << " " << twb(2) << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() <<" "
            << ba(0) << " "
            << ba(1) << " "
            << ba(2) << " "
            << bg(0) << " "
