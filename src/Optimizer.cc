...
#include "Gyro_bias_factor.hpp"
#include "Translation_factor.hpp"
#include "Converter.h"
...
int Optimizer::PoseOptimization(Frame *pFrame)
{
    ...
    for(size_t it=0; it<4; it++)
    {
        ...
        optimizer.initializeOptimization(0); //only optimize the edge that level equals 0, default is 0
...
int Optimizer::PoseOptimization(KeyFrame* pCurKF)
{
    g2o::SparseOptimizer optimizer;
    g2o::BlockSolverX::LinearSolverType * linearSolver;
    ...
    // Set Frame vertex
    // g2o::VertexSE3Expmap * vSE3 = new g2o::VertexSE3Expmap();
    // Sophus::SE3<float> Tcw = pCurKF->GetPose();
    // vSE3->setEstimate(g2o::SE3Quat(Tcw.unit_quaternion().cast<double>(),Tcw.translation().cast<double>()));

    VertexPose3DoF* vSE3 = new VertexPose3DoF(pCurKF);
    Sophus::SE3<float> Tcw = pCurKF->GetPose();
    vSE3->setId(0);
    vSE3->setFixed(false);
    optimizer.addVertex(vSE3);
    ...
    // Set MapPoint vertices
    const int N = pCurKF->N;

    vector<EdgeSE3ProjectXYZOnlyPose2*> vpEdgesMono;
    vector<ORB_SLAM3::EdgeSE3ProjectXYZOnlyPoseToBody *> vpEdgesMono_FHR;
    vector<size_t> vnIndexEdgeMono, vnIndexEdgeRight;
    vpEdgesMono.reserve(N);
    vpEdgesMono_FHR.reserve(N);
    ...
    // vector<g2o::EdgeStereoSE3ProjectXYZOnlyPose*> vpEdgesStereo;
    vector<EdgeStereoSE3ProjectXYZOnlyPose2*> vpEdgesStereo;
    vector<size_t> vnIndexEdgeStereo;
    vpEdgesStereo.reserve(N);
    vnIndexEdgeStereo.reserve(N);
    ...
    vector<bool> vbOutlier = vector<bool>(N,false);
    ...
    for(int i=0; i<N; i++)
    {
        MapPoint* pMP = pCurKF->GetMapPoint(i);
        if(pMP)
        {
            //Conventional SLAM
            if(!pCurKF->mpCamera2){
                // Monocular observation
                if(pCurKF->mvuRight[i]<0)
                {
                    nInitialCorrespondences++;
                    vbOutlier[i] = false;

                    Eigen::Matrix<double,2,1> obs;
                    const cv::KeyPoint &kpUn = pCurKF->mvKeysUn[i];
                    obs << kpUn.pt.x, kpUn.pt.y;


                    EdgeSE3ProjectXYZOnlyPose2* e = new EdgeSE3ProjectXYZOnlyPose2();

                    e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(0)));
                    e->setMeasurement(obs);
                    const float invSigma2 = pCurKF->mvInvLevelSigma2[kpUn.octave];
                    e->setInformation(Eigen::Matrix2d::Identity()*invSigma2);
                    ...
                    e->pCamera = pCurKF->mpCamera;
                    e->Xw = pMP->GetWorldPos().cast<double>();
                    e->Rcw = pCurKF->GetRotation().cast<double>();
                    optimizer.addEdge(e);
                ...
                else  // Stereo observation
                {
                    ...
                    Eigen::Matrix<double,3,1> obs;
                    const cv::KeyPoint &kpUn = pCurKF->mvKeysUn[i];
                    const float &kp_ur = pCurKF->mvuRight[i];
                    obs << kpUn.pt.x, kpUn.pt.y, kp_ur;

                    // g2o::EdgeStereoSE3ProjectXYZOnlyPose* e = new g2o::EdgeStereoSE3ProjectXYZOnlyPose();
                    EdgeStereoSE3ProjectXYZOnlyPose2* e = new EdgeStereoSE3ProjectXYZOnlyPose2();
    ...
    {
        Tcw = pCurKF->GetPose();
        vSE3->setEstimate(Tcw.translation().cast<double>());
        ...
        {
            // ORB_SLAM3::EdgeSE3ProjectXYZOnlyPose* e = vpEdgesMono[i];
            EdgeSE3ProjectXYZOnlyPose2* e = vpEdgesMono[i];
            const size_t idx = vnIndexEdgeMono[i];
        ...
        {
            // g2o::EdgeStereoSE3ProjectXYZOnlyPose* e = vpEdgesStereo[i];
            EdgeStereoSE3ProjectXYZOnlyPose2* e = vpEdgesStereo[i];
    ...
    VertexPose3DoF* vtrans_recov = static_cast<VertexPose3DoF*>(optimizer.vertex(0));
    Eigen::Vector3d trans_recov = vtrans_recov->estimate();
    Sophus::SE3<float> pose(pCurKF->GetRotation(),
            trans_recov.cast<float>());
    pCurKF->SetPose(pose);
...
void Optimizer::InertialOptimization(Map *pMap, Eigen::Matrix3d &Rwg, double &scale, Eigen::Vector3d &bg, Eigen::Vector3d &ba, bool bMono, Eigen::MatrixXd  &covInertial, bool bFixedVel, bool bGauss, float priorG, float priorA, bool isUpdate)
{
    ...
    std::cout<<"isUpdate: "<<isUpdate<<std::endl;
    if(isUpdate)
    {
...
std::vector<Eigen::Matrix3d> Optimizer::summation(const std::vector<Eigen::Vector3f> &bearingVectori, const std::vector<Eigen::Vector3f> &bearingVectorj)
{
	std::vector<Eigen::Matrix3d> summationVec(6);
	...
	for (int i = 0; i < (int) bearingVectori.size(); i++) 
	{
		Eigen::Vector3d f1 = bearingVectori[i].cast<double>();
		Eigen::Vector3d f2 = bearingVectorj[i].cast<double>();
	...
	summationVec[0] = xxF, summationVec[1] = yyF, summationVec[2] = zzF;
	summationVec[3] = xyF, summationVec[4] = yzF, summationVec[5] = zxF;
	return summationVec;
}

void Optimizer::GyroscopeBiasOptimizer(Map *pMap, Eigen::Vector3d &bg, float &err)
{
    long unsigned int maxKFid = pMap->GetMaxKFid();
    const vector<KeyFrame*> vpKFs = pMap->GetAllKeyFrames();

    double bg_est[3] = {bg[0], bg[1], bg[2]};
    ceres::Problem problem;
    Eigen::Matrix3f Rcb = vpKFs[0]->mImuCalib.mTcb.rotationMatrix();
    Eigen::Matrix3d Rbc = (Rcb.transpose()).cast<double>();
    const float &fx = vpKFs[0]->fx;
    const float &fy = vpKFs[0]->fy;
    const float &cx = vpKFs[0]->cx;
    const float &cy = vpKFs[0]->cy;

    // Define an array of colors
    std::vector<cv::Scalar> colors = {cv::Scalar(0, 255, 0), cv::Scalar(255, 0, 0), cv::Scalar(0, 0, 255), cv::Scalar(255, 255, 0), cv::Scalar(255, 0, 255)};
    std::vector<float> Err;
    for(size_t i=0;i<vpKFs.size();i++)
    {
        // std::cout<<"Process frame id: "<<i<<std::endl;
        KeyFrame* pKFj = vpKFs[i];
   
        if(pKFj->mPrevKF && pKFj->mnId<=maxKFid)
        {
            if(pKFj->isBad() || pKFj->mPrevKF->mnId>maxKFid)
                continue;

            KeyFrame* pKFi = pKFj->mPrevKF;
            std::vector<Eigen::Vector3f> bearingVectori, bearingVectorj;
            
            Eigen::Matrix3d dR_dbg = (pKFj->mpImuPreintegrated->JRg).cast<double>();
            Eigen::Matrix3d gamma = (pKFj->mpImuPreintegrated->GetUpdatedDeltaRotation()).cast<double>();
            Eigen::Matrix3d gamma_d = gamma.cast<double>();
            Eigen::Vector3d twci = (pKFi->GetPoseInverse().translation()).cast<double>();
            Eigen::Matrix3d Rwci = (pKFi->GetPoseInverse().rotationMatrix()).cast<double>();
            Eigen::Matrix3d Rciw = Rwci.transpose();
            Eigen::Vector3d twcj = (pKFj->GetPoseInverse().translation()).cast<double>();
            Eigen::Vector3d tcicj = Rciw  * (twcj - twci);

            // Optical flow tracking
            std::vector<cv::Point2f> prev_pts, cur_pts;
            for (int k = 0; k < (int)pKFi->mvKeysUn.size(); k++)
            {
                const cv::KeyPoint &kpUni = pKFi->mvKeysUn[k];
                prev_pts.push_back(cv::Point2f(kpUni.pt.x, kpUni.pt.y));
            }

            std::vector<uchar> status;
            std::vector<float> err;
            cv::calcOpticalFlowPyrLK(pKFi->imgLeft, pKFj->imgLeft, prev_pts, cur_pts, status, err, cv::Size(21, 21), 3);

            std::vector<uchar> reverse_status;
            std::vector<cv::Point2f> reverse_pts = prev_pts;
            cv::calcOpticalFlowPyrLK(pKFj->imgLeft, pKFi->imgLeft, cur_pts, reverse_pts, reverse_status, err, cv::Size(21, 21), 1, 
            cv::TermCriteria(cv::TermCriteria::COUNT+cv::TermCriteria::EPS, 30, 0.01), cv::OPTFLOW_USE_INITIAL_FLOW);
     
            for(size_t k = 0; k < status.size(); k++)
            {
                if(status[k] && reverse_status[k] && distance(prev_pts[k], reverse_pts[k]) <= 0.5)
                {
                    status[k] = 1;
                }
                else
            ...
            for (int k = 0; k < (int)cur_pts.size(); k++)
            {
                cv::Point2f pt1 = prev_pts[k];
                float x = (pt1.x - cx) / fx;
                float y = (pt1.y - cy) / fy;
                Eigen::Vector3f bvi(x, y, 1);
                bvi.normalize();

                bearingVectori.push_back(bvi);

                cv::Point2f pt2 = cur_pts[k];
                x = (pt2.x - cx) / fx;
                y = (pt2.y - cy) / fy;
                Eigen::Vector3f bvj(x, y, 1);
                bvj.normalize();
                bearingVectorj.push_back(bvj);
            }
            ...
            if(cur_pts.size() >= 15)
            {
                ...
                cv::findFundamentalMat(prev_pts, cur_pts, cv::FM_RANSAC, 0.1, 0.999, status_left);
                reduceVector(bearingVectori, status_left);
                reduceVector(bearingVectorj, status_left);
                reduceVector(prev_pts, status_left);
                reduceVector(cur_pts, status_left);
                
                if(bearingVectori.size()>=15)
                {
                    std::vector<Eigen::Matrix3d> summationVec =  summation(bearingVectori, bearingVectorj);
                    problem.AddResidualBlock(new ceres::AutoDiffCostFunction<GyroScopeBiasEdgeFactor, 1, 3>( 
                    new GyroScopeBiasEdgeFactor(summationVec[0], summationVec[1], summationVec[2], summationVec[3],summationVec[4],summationVec[5], dR_dbg, gamma, Rbc)),
                    nullptr,            
                    bg_est);
                }  
            }
            ...
            float err_total = 0.0;
            float err_total2 = 0.0;
            for(int k = 0; k < bearingVectori.size(); k++)
            {
                Eigen::Vector3d fi = bearingVectori[k].cast<double>();
                Eigen::Vector3d fj = bearingVectori[k].cast<double>();
                Eigen::Matrix3d skewfi;
                skewfi << 0.0, -fi(2), fi(1),
                    fi(2), 0.0, -fi(0),
                    -fi(1), fi(0), 0.0;
                
                Eigen::Vector3d normal_k = skewfi * Rbc.transpose()*gamma_d*Rbc*fj;
                Eigen::Vector3f normal = normal_k.cast<float>();

                err_total += twci.cast<float>().transpose() * normal;
            }
            err_total2 = err_total / bearingVectori.size();
            // cout<<err_total2<<endl;
            Err.push_back(err_total2);
        }
    }

    ceres::Solver::Options options;     
	options.linear_solver_type = ceres::DENSE_NORMAL_CHOLESKY; //ceres::DENSE_QR;
	options.minimizer_progress_to_stdout = false; //true   

	ceres::Solver::Summary summary;                
	ceres::Solve(options, &problem, &summary); 

	bg << bg_est[0], bg_est[1], bg_est[2];
	// std::cout<<"Our estimated bg: "<<bg.transpose()<<std::endl;
    Eigen::Vector3f ba = vpKFs[0]->GetAccBias();
    
    for (int i = 0; i < Err.size(); i++)
    {
        err += Err[i]/vpKFs.size();
    }
    cout<<err<<endl;
    //Keyframes velocities and biases
    const int N = vpKFs.size();
    IMU::Bias bias (ba[0],ba[1],ba[2], bg_est[0],bg_est[1],bg_est[2]);
    for(size_t i=0; i<N; i++)
    {
        KeyFrame* pKFi = vpKFs[i];
        if(pKFi->mnId>maxKFid)
...
void Optimizer::EstimateTranslation(const vector<KeyFrame*> &vpKFs)
{
    int N = vpKFs.size();
    Eigen::Matrix3f Rcb = vpKFs[0]->mImuCalib.mTcb.rotationMatrix();
    Eigen::Matrix3d Rbc = (Rcb.transpose()).cast<double>();
    ...
    // Method 1
    // ceres::Problem problem;
    // double rel_trans[N-1][3];

    // for(size_t i=1;i<vpKFs.size();i++)
    // {
    //     KeyFrame* pKFi = vpKFs[i-1];
    //     KeyFrame* pKFj = vpKFs[i];

    //     if (pKFj->mpImuPreintegrated)
    //     {
    //         const Eigen::Vector3f Gz(0, 0, -IMU::GRAVITY_VALUE);

    //         const Eigen::Vector3f twb1 = pKFj->mPrevKF->GetImuPosition();
    //         const Eigen::Matrix3f Rwb1 = pKFj->mPrevKF->GetImuRotation();
    //         const Eigen::Vector3f Vwb1 = pKFj->mPrevKF->GetVelocity();
    //         float t12 = pKFj->mpImuPreintegrated->dT;

    //         const Eigen::Vector3f twb2 = pKFj->GetImuPosition();
    //         const Eigen::Vector3f Vwb2 = pKFj->GetVelocity();
            
            
    //         pKFj->SetImuPoseVelocity(IMU::NormalizeRotation(Rwb1*pKFj->mpImuPreintegrated->GetUpdatedDeltaRotation()),
    //                                     twb2, //twb1 + Vwb1*t12 + 0.5f*t12*t12*Gz+ Rwb1*pKFj->mpImuPreintegrated->GetUpdatedDeltaPosition(),
    //                                     Vwb1 + Gz*t12 + Rwb1*pKFj->mpImuPreintegrated->GetUpdatedDeltaVelocity());        
    //     }

    //     Eigen::Matrix3f gamma = pKFj->mpImuPreintegrated->GetUpdatedDeltaRotation();
    //     Eigen::Matrix3d gamma_d = gamma.cast<double>();
    //     std::vector<Eigen::Vector3f> bearingVectoriLeft, bearingVectorjLeft;

    //     Eigen::Vector3f twci = pKFi->GetPoseInverse().translation();
    //     Eigen::Matrix3f Rwci = pKFi->GetPoseInverse().rotationMatrix();
    //     Eigen::Matrix3f Rciw = Rwci.transpose();
    //     Eigen::Vector3f twcj = pKFj->GetPoseInverse().translation();
    //     Eigen::Vector3f tcicj = Rciw  * (twcj - twci);
    //     rel_trans[i-1][0] = tcicj[0];
    //     rel_trans[i-1][1] = tcicj[1];
    //     rel_trans[i-1][2] = tcicj[2];

    //     // problem.AddParameterBlock(rel_trans[i-1], 3);
    //     // std::cout<<pKFi->mnId<<"->"<<pKFj->mnId<<"tcicj (orb3): "<<tcicj.transpose()<<std::endl;
    //     // Optical flow tracking
    //     std::vector<cv::Point2f> prev_pts, cur_pts;
    //     for (int k = 0; k < (int)pKFi->mvKeysUn.size(); k++)
    //     {
    ...
    //         Eigen::Vector3f bvi(x, y, 1);
    //         bvi.normalize();
    //         bearingVectoriLeft.push_back(bvi);
    ...
    //         Eigen::Vector3f bvj(x, y, 1);
    //         bvj.normalize();
    //         bearingVectorjLeft.push_back(bvj);
    ...
    //         std::vector<uchar> status_left;
    //         cv::findFundamentalMat(prev_pts, cur_pts, cv::FM_RANSAC, 0.1, 0.999, status_left);
    //         reduceVector(bearingVectoriLeft, status_left);
    //         reduceVector(bearingVectorjLeft, status_left);
    //         reduceVector(prev_pts, status_left);
    //         reduceVector(cur_pts, status_left);
    ...
    //     int n_bearing_vector = bearingVectoriLeft.size();
    //     Eigen::MatrixXd tmp_A(n_bearing_vector, 3);
    //     tmp_A.setZero();

    //     Eigen::VectorXd tmp_b(n_bearing_vector);
    //     tmp_b.setZero();
    //     float err_totoal = 0.0;
    //     float err_total2 = 0.0;
    //     for(int k = 0; k < n_bearing_vector; k++)
    //     {
    //         Eigen::Vector3d fi = bearingVectoriLeft[k].cast<double>();
    //         Eigen::Vector3d fj = bearingVectorjLeft[k].cast<double>();
    //         Eigen::Matrix3d skewfi;
    //         skewfi << 0.0, -fi(2), fi(1),
    //             fi(2), 0.0, -fi(0),
    //             -fi(1), fi(0), 0.0;
    ...
    //         Eigen::Vector3d normal_k = skewfi * Rbc.transpose()*gamma_d*Rbc*fj;
    //         Eigen::Vector3f normal = normal_k.cast<float>();
    //         err_totoal += tcicj.transpose() * normal;
    //         Eigen::Vector3f t(1,2,3);
    //         err_total2 += t.transpose() * normal;
    //         tmp_A.block<1, 3>(k, 0) = normal_k.transpose();

    //         //
    //         // problem.AddResidualBlock(new ceres::AutoDiffCostFunction<TranslationFactor, 1, 3>( 
    //         // new TranslationFactor(normal_k[0], normal_k[1], normal_k[2])),
    //         // nullptr,            
    //         // rel_trans[i-1]);
    //     }

    //     // std::cout<<"avg err: "<<err_totoal/n_bearing_vector<<", total err: "<<err_totoal<<std::endl;
    //     // std::cout<<"avg err: "<<err_total2/n_bearing_vector<<", total err: "<<err_total2<<std::endl;
    ...
    //     // cv::Mat A_CV;
    //     // cv::eigen2cv(A, A_CV);
    //     // cv::SVDecomp(A_CV, w,u,vt,cv::SVD::MODIFY_A | cv::SVD::FULL_UV);
    //     // cv::Mat x = vt.row(2).t();
    //     // Eigen::Vector3d t;
    //     // cv::cv2eigen(x, t);
    //     // std::cout<<t.transpose()<<", "<<tcicj.normalized().transpose()<<std::endl;
    ...
    //     }
    //     x = x*norm;
    //     std::cout<<"tcicj: "<<x.transpose()<<", "<<tcicj.normalized().transpose()<<std::endl;
    //     // std::cout<<"compare: "<<t.transpose()<<", "<<tcicj.transpose()<<std::endl;
    //     std::cout<<"compare: "<<x.transpose()<<", "<<tcicj.transpose()<<std::endl;


  

    //     // Update translation and mappoints
    //     std::cout<<"twcj (before): "<<pKFj->GetPoseInverse().translation().transpose()<<std::endl;
    //     Eigen::Matrix3f Rwcj = pKFj->GetPoseInverse().rotationMatrix();
    //     Eigen::Vector3f twcj_new = Rwci * (x.cast<float>()) + twci;

    //     Sophus::SE3f Twcj = Sophus::SE3f(Rwcj, twcj_new);
    //     Sophus::SE3f Tcjw = Twcj.inverse();
    //     pKFj->SetPose(Tcjw);
    //     std::cout<<"twcj (after): "<<pKFj->GetPoseInverse().translation().transpose()<<std::endl;
...
void Optimizer::reduceVector(std::vector<Eigen::Vector3f> &v, std::vector<uchar> status)
{
    int j = 0;
...
void Optimizer::reduceVector(std::vector<cv::Point2f> &v, std::vector<uchar> status)
{
    int j = 0;
    for (int i = 0; i < int(v.size()); i++)
        if (status[i])
...
double Optimizer::distance(cv::Point2f &pt1, cv::Point2f &pt2)
{
...
bool Optimizer::LinearLooselyCoupledAlignment(const vector<KeyFrame*> &vpKFs)
{
    int N = vpKFs.size();
    int n_state = N * 3 + 3; // v0~vn, gw
    Eigen::MatrixXf A{n_state, n_state};
    A.setZero();
    Eigen::VectorXf b{n_state};
    b.setZero();
    ...
    for(size_t i=0;i<vpKFs.size();i++)
    {
        ...
        if(pKFj->mPrevKF)
        {
            ...
            double dt = pKFj->mpImuPreintegrated->dT;
            Eigen::Vector3f alpha = pKFj->mpImuPreintegrated->GetUpdatedDeltaPosition();
            Eigen::Vector3f beta = pKFj->mpImuPreintegrated->GetUpdatedDeltaVelocity();
            Eigen::Matrix3f Rbiw = pKFi->GetImuRotation().transpose();
            Eigen::Matrix3f Rwbj = pKFj->GetImuRotation();
            Eigen::Matrix3f gamma = Rbiw * Rwbj;
            std::vector<Eigen::Vector3f> mapPointsInLefti, mapPointsInLeftj;

            // ORB descriptor based tracking
            ORBmatcher matcher(0.1,true);
            std::vector<int> vMatches12;
            int nmatches = matcher.SearchByBoW(pKFi, pKFj, vMatches12);
            std::cout<<"nmatches: "<<nmatches<<std::endl;
            for (int k = 0; k < (int)vMatches12.size(); k++)
            {
                if(vMatches12[k] < 0 || pKFi->mvDepth[k]<0) 
                    continue;

                int index_j = vMatches12[k];
                if(pKFj->mvDepth[index_j]<0)
                    continue;

                float zi = pKFi->mvDepth[k];
                float zj = pKFj->mvDepth[index_j];
                if(zi>0 && zj>0)
                {
                    Eigen::Vector3f x3Di;
                    pKFi->UnprojectStereoInBodyFrame(k, x3Di);
                    mapPointsInLefti.push_back(x3Di);

                    Eigen::Vector3f x3Dj;
                    pKFj->UnprojectStereoInBodyFrame(index_j, x3Dj);
                    mapPointsInLeftj.push_back(x3Dj);
                }
            }

            const int nMapPoints = mapPointsInLefti.size();
            Eigen::MatrixXf tmp_A(nMapPoints*3+3, 9);
            tmp_A.setZero();

            Eigen::VectorXf tmp_b(nMapPoints*3+3);
            tmp_b.setZero();

            for (int k = 0; k < nMapPoints; k++) 
            {
                Eigen::Vector3f xi = mapPointsInLefti[k];
                Eigen::Vector3f xj = mapPointsInLeftj[k];
         

                // without considering acc bias
                tmp_A.block<3, 3>(3*k, 0) = Eigen::Matrix3f::Identity() * dt;
                tmp_A.block<3, 3>(3*k, 6) = 0.5* dt * dt *Eigen::Matrix3f::Identity()* Rbiw;
                tmp_b.block<3, 1>(3*k, 0) = xi - gamma * xj - alpha;
            }

            tmp_A.block<3, 3>(3*nMapPoints, 0) = -Eigen::Matrix3f::Identity();
            tmp_A.block<3, 3>(3*nMapPoints, 3) = gamma;
            tmp_A.block<3, 3>(3*nMapPoints, 6) = -dt * Eigen::Matrix3f::Identity() * Rbiw;
    
            tmp_b.block<3, 1>(3*nMapPoints, 0) = beta;

            Eigen::MatrixXf r_A = tmp_A.transpose()  * tmp_A;
            Eigen::VectorXf r_b = tmp_A.transpose()  * tmp_b;
    ...
    Eigen::Vector3f g = x.segment<3>(n_state - 3);
    cout << "Step2's g     " << g.norm() << " " << g.transpose() <<endl;
    if(fabs(g.norm() - 9.81) > 1.0) {
        return false;
    }
    return true;   
}

void Optimizer::SolveGyroscopeBiasVins(const vector<KeyFrame*> &vpKF, Map *pMap, Eigen::Vector3d &bg) {
    Eigen::Matrix3d A;
    Eigen::Vector3d b;
    Eigen::Vector3d delta_bg;
    A.setZero();
    b.setZero();
    double curKFTimeStamp = 1e9 * vpKF[0]->mTimeStamp;
    string imgName = to_string(curKFTimeStamp);
    string imgName_prefix = imgName.substr(0, imgName.find("."));
    std::cout<<vpKF[0]->mnId<<": "<<imgName_prefix<<std::endl;
    for (size_t i =1; i < vpKF.size(); i++)
    {
        double curKFTimeStamp = 1e9 * vpKF[i]->mTimeStamp;
        string imgName = to_string(curKFTimeStamp);
        string imgName_prefix = imgName.substr(0, imgName.find("."));
        std::cout<<vpKF[i]->mnId<<": "<<imgName_prefix<<std::endl;

        KeyFrame* frame_i = vpKF[i-1];
        KeyFrame* frame_j = vpKF[i];
        ...
        Eigen::Quaterniond q_ij((frame_i->GetImuRotation().transpose() * frame_j->GetImuRotation()).cast<double>());
        tmp_A = frame_j->mpImuPreintegrated->JRg.cast<double>();
        Eigen::Quaterniond delta_q((frame_j->mpImuPreintegrated->GetUpdatedDeltaRotation()).cast<double>());
    ...
    bg = A.ldlt().solve(b);
    // Eigen::Vector3d vins_bg = vpKF[0]->GetGyroBias().cast<double>() + delta_bg;
    //Keyframes velocities and biases
    const int N = vpKF.size();
    Eigen::Vector3f ba = vpKF[0]->GetAccBias();
    IMU::Bias bias (ba[0],ba[1],ba[2], bg[0],bg[1],bg[2]);
    ...
    cout<<"vins bg: "<<bg.transpose()<<endl;
