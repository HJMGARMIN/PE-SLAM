        ...
        if(CheckNewKeyFrames() && !mbBadImu)
        {
            ...
            {           
                ...
                if(!mpCurrentKeyFrame->GetMap()->isImuInitialized() && mbInertial)
                {
                    ...
                    {
                        InitializeIMU(1e2, 1e5, true, true); //ours true,true; orb-slam3 true, false
                    }
                        ...
                        if(!mpCurrentKeyFrame->GetMap()->GetIniertialBA1()){
                            if (mTinit>5.0f)
                            {
                                ...
                                if (mbMonocular)
                                    ...
                                    std::cout<<"(WWH)+++ Second times VI-INIT+++"<<std::endl;
                                    InitializeIMU(1.f, 1e5, true);
                                }
                            ...
                            if (mTinit>15.0f){
                                ...
                                if (mbMonocular)
                                    ...
                                    std::cout<<"(WWH)+++ Third times VI-INIT+++"<<std::endl;
                                    InitializeIMU(0.f, 0.f, true);
                                }
...
void LocalMapping::InitializeIMU(float priorG, float priorA, bool bFIBA, bool isUsed)
{
    if (mbResetRequested)
        return;
    ...
    if (!mpCurrentKeyFrame->GetMap()->isImuInitialized())
    {
        ...
        std::cout<<"mTinit: "<<mTinit<<std::endl;
    }
    else
    {
    ...
    double curKFTimeStamp = 1e9 * mpCurrentKeyFrame->mTimeStamp;
    string imgName = to_string(curKFTimeStamp);
    string imgName_prefix = imgName.substr(0, imgName.find("."));
    cout<<" +++ Current id: " << to_string(mpCurrentKeyFrame->mnId) <<" and name:"<< imgName_prefix <<"+++"<< endl;
    ...
    if(!isUsed)
    {
        std::chrono::steady_clock::time_point step_st = std::chrono::steady_clock::now();
        Optimizer::InertialOptimization(mpAtlas->GetCurrentMap(), mRwg, mScale, mbg, mba, mbMonocular, infoInertial, false, false, priorG, priorA, !isUsed); 
        // std::cout<<"ORB's estimated bg: "<<mbg.transpose()<<std::endl;
        std::chrono::steady_clock::time_point step_end = std::chrono::steady_clock::now();
        std::chrono::duration<double, std::milli>  step_dur = std::chrono::duration<double,std::milli>(step_end-step_st);
        double cost_t = step_dur.count();
        std::cout<<"[ORB-SLAM3] Estimate bias, g, vecl takes: "<<cost_t<<" ms"<<std::endl;
    }

    if(isUsed)
    {
        // Step 1: Estimate gyroscope bias firstly.
        std::chrono::steady_clock::time_point step_st1 = std::chrono::steady_clock::now();
        Optimizer::GyroscopeBiasOptimizer(mpAtlas->GetCurrentMap(), mbg, AvgErr);
        // std::cout<<"Ours estimated bg: "<<mbg.transpose()<<std::endl;
        std::chrono::steady_clock::time_point step_end1 = std::chrono::steady_clock::now();
        std::chrono::duration<double, std::milli>  step_dur = std::chrono::duration<double,std::milli>(step_end1-step_st1);
        double cost_t = step_dur.count();
        std::cout<<"[Ours] Estimate gyroscope bias takes: "<<cost_t<<" ms"<<std::endl;

        Eigen::Vector3f bg = mbg.cast<float>();
        ImuInitializer imuInit = ImuInitializer(vpKF, bg);
        if(imuInit.init_imu())
        {
            mba = imuInit.ba.cast<double>();

            IMU::Bias b_(imuInit.ba(0), imuInit.ba(1), imuInit.ba(2), bg(0), bg(1), bg(2));
            // update bias
            ...
            cout<<"Analytial Solution Finished"<<endl;
        }

        // Step 2: Joint refine
        std::chrono::steady_clock::time_point step_st2 = std::chrono::steady_clock::now();
        Optimizer::InertialOptimization(mpAtlas->GetCurrentMap(), mRwg, mScale, mbg, mba, mbMonocular, infoInertial, false, false, priorG, priorA, true);
        // std::cout<<"Ours estimated bg (after step 2): "<<mbg.transpose()<<std::endl;
        std::chrono::steady_clock::time_point step_end2 = std::chrono::steady_clock::now();
        std::chrono::duration<double, std::milli>  step_dur2 = std::chrono::duration<double,std::milli>(step_end2-step_st2);
        double cost_t2 = step_dur2.count();
        std::cout<<"[Ours] Estimate bias, g, vecl takes: "<<cost_t2<<" ms"<<std::endl;
    }
    ...
    if (AvgErr>1e-4)
    {
        cout << "IMU initailize failure" << endl;
        bInitializing=false;
        return;
    }
    ...
    // Step3: Integrate rotation and optimize translation only
    if(isUsed)
    {
        std::chrono::steady_clock::time_point step_st = std::chrono::steady_clock::now();
        Optimizer::EstimateTranslation(vpKF);
        std::chrono::steady_clock::time_point step_end = std::chrono::steady_clock::now();
        std::chrono::duration<double, std::milli>  step_dur = std::chrono::duration<double,std::milli>(step_end-step_st);
        double cost_t = step_dur.count();
        std::cout<<"[Ours] Integrate rotation and optimize translation only takes: "<<cost_t<<" ms"<<std::endl;
    }
        
    // Change to your save path for saving results
    string out_file_name = "results/init_traj(" + to_string(mpCurrentKeyFrame->mnId) + ")imuonly.csv";
    SaveTrajectory(vpKF, out_file_name);
    ...
    std::cout<<"Estimated bg (BA): "<<mpCurrentKeyFrame->GetGyroBias().transpose()<<std::endl;
    std::cout<<"Estimated ba (BA): "<<mpCurrentKeyFrame->GetAccBias().transpose()<<std::endl;
    out_file_name = "results/init_traj(" + to_string(mpCurrentKeyFrame->mnId) + ")viba.csv"; 
    SaveTrajectory(vpKF, out_file_name);
...
//TUM
void LocalMapping::SaveTrajectory(vector<KeyFrame*> vpKF, const string out_file_name) 
{
    ofstream f;
...
void LocalMapping::SaveGyroscopeBias(const Eigen::Vector3d &bg, const string out_file_name) 
{
    ofstream f;
    f.open(out_file_name.c_str());
    f<<fixed;
    f <<  setprecision(9) << bg(0) << " " << bg(1) << " " <<bg(2) <<  endl;
    f.close();
}

void LocalMapping::SaveComputationTime(const double &cost_t, const string out_file_name)
{
    // std::ofstream file(out_file_name, std::ios::app);
    ofstream file;
...
string LocalMapping::Double2String(double val, int fixed) {
    ostringstream oss;
    oss<< fixed << setprecision(fixed) << val;
    return oss.str();
}
