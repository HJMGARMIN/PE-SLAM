...
namespace ORB_SLAM3
{
    ...
    int ORBmatcher::SearchForInitialization(Frame &F1, Frame &F2, vector<cv::Point2f> &vbPrevMatched, vector<int> &vnMatches12, int windowSize)
    {
        ...
        tbb::parallel_for(tbb::blocked_range<std::size_t>(0,F1.mvKeysUn.size()),[&](tbb::blocked_range<std::size_t>rF1mvKeysUn)
        {
    ...
    int ORBmatcher::SearchByBoW(KeyFrame *pKF1, KeyFrame* pKF2,  std::vector<int> &vMatches12)
    {
        const vector<cv::KeyPoint> &vKeysUn1 = pKF1->mvKeysUn;
        const DBoW2::FeatureVector &vFeatVec1 = pKF1->mFeatVec;
        const vector<MapPoint*> vpMapPoints1 = pKF1->GetMapPointMatches();
        ...
        vMatches12 = vector<int>(vpMapPoints1.size(), -1);
        vector<bool> vbMatched2(vpMapPoints2.size(),false);
